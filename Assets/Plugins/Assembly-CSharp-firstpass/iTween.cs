...
public class iTween : MonoBehaviour
{
	...
	{
		delayStarted = Time.timeAsDouble;
	...
	public static void Pause(GameObject target)
	{
		...
		for (int i = 0; i < components.Length; i++)
		{
			...
			if (iTween2.delay > 0f)
			{
				iTween2.delay -= (float)(Time.timeAsDouble - iTween2.delayStarted);
	...
	public static void Stop(GameObject target, string type)
	{
		...
		for (int i = 0; i < components.Length; i++)
		{
			...
			if (iTween2.IsValid)
			{
				...
				{
					Debug.LogError("Error with iTween Stop. targetType=" + text + ", type=" + type);
	...
	private static void Launch(GameObject target, Hashtable args)
	{
		...
		target.AddComponent<iTween>().id = (string)args["id"];
	}
	...
	private void UpdatePercentage()
	{
		if (useRealTime)
		{
			runningTime += (float)(Time.realtimeSinceStartupAsDouble - lastRealTime);
		}
		else
		{
