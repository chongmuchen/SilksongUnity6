...
namespace UnityStandardAssets.ImageEffects
{
	...
	public class BlurOptimized : PostEffectsBase
	{
		...
		public void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			RenderTexture renderTexture = RenderTexture.GetTemporary(width, height, 24, source.format);
			...
			for (int i = 0; i < blurIterations; i++)
			{
				...
				RenderTexture temporary = RenderTexture.GetTemporary(width, height, 24, source.format);
				temporary.filterMode = FilterMode.Bilinear;
				Graphics.Blit(renderTexture, temporary, blurMaterial, 1 + num2);
				RenderTexture.ReleaseTemporary(renderTexture);
				renderTexture = temporary;
				temporary = RenderTexture.GetTemporary(width, height, 24, source.format);
				temporary.filterMode = FilterMode.Bilinear;
