// Sprite texture with vertex tint: Difference.
...
float4 UIBlendFragment(UIBlendFragmentInput input) : SV_Target0
{
    ...
    float3 colorDifference = mad((-spriteSample.xyz), input.color.xyz, backgroundColor);
    float opacity = spriteSample.w * input.color.w;
    outputColor.w = opacity;
    outputColor.xyz = abs(colorDifference);
    return outputColor;
}
