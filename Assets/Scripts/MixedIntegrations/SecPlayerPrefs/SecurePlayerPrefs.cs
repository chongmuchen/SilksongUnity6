...
namespace SecPlayerPrefs
{
	public class SecurePlayerPrefs : ScriptableObject
	{
		...
		{
			PlayerPrefs.SetString(Encrypt(Key), Encrypt(Value.ToString(CultureInfo.InvariantCulture)));
		}
		...
		{
			string text = PlayerPrefs.GetString(Encrypt(Key));
			if (text == "")
			{
				return "";
			}
		...
		public static float GetFloat(string Key)
		{
			...
			return float.Parse(Decrypt(text), NumberStyles.Float, CultureInfo.InvariantCulture);
		}
		...
		public static bool GetBool(string Key)
		{
			...
			return bool.Parse(Decrypt(text));
		}
