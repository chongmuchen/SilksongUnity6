...
public class InventoryMapManager : InventoryItemManager, IInventoryPaneAvailabilityProvider
{
	...
	protected override IEnumerable<InventoryItemSelectable> DefaultSelectables
	{
		get
		{
			...
			return from item in array
				where item.IsUnlocked && item.gameObject.activeSelf
	...
	{
		if (HeroController.HasNoMap(gameMap))
		{
			return false;
		}
	...
	private void OnPaneStart()
	{
		...
		UpdateKeyPromptState(wasManual: false);
		bool flag = HeroController.HasNoMap(gameMap);
		noMapSymbol.SetActive(flag);
		hasMapParent.SetActive(!flag);
	}

	private void UpdateKeyPromptState(bool wasManual)
	{
		if (PlayerData.instance.HasAnyPin && !HeroController.HasNoMap(gameMap))
		{
