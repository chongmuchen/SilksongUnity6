...
public class DialogueYesNoBox : YesNoBox
{
	...
	public static void Open(Action yes, Action no, bool returnHud, string text, CurrencyType currencyType, int currencyAmount, IReadOnlyList<SavedItem> items, IReadOnlyList<int> amounts, bool displayHudPopup, bool consumeCurrency, SavedItem willGetItem, TakeItemTypes takeItemType = TakeItemTypes.Silent, YesNoAction.DisplayType displayType = YesNoAction.DisplayType.RequiredItems)
	{
		if (!_instance)
		{
			return;
		...
		if (items != null && items.Count > 0)
		{
			...
			if (displayType == YesNoAction.DisplayType.RequiredItems)
			{
			...
			{
				foreach (SavedItem item5 in items)
				{
					if (item5 is CollectableItem item2)
					{
		...
		else if (displayType == YesNoAction.DisplayType.WillGetItems)
		{
			while (_instance.instantiatedItems.Count < 1)
			{
				_instance.instantiatedItems.Add(UnityEngine.Object.Instantiate(_instance.itemTemplate, _instance.itemTemplate.transform.parent));
			}
			SetupWillGetItemDisplay();
		...
		Action yes2 = delegate
		{
			...
			else
			{
				...
				if (items != null && takeItemType == TakeItemTypes.Silent)
				{
					...
					{
						if (item6 is CollectableItem item4)
						{
		...
		Action no2 = delegate
		{
			if (displayHudPopup)
			{
				...
				if (items != null && takeItemType == TakeItemTypes.Silent)
				{
					...
					{
						if (item7 is CollectableItem item3)
						{
							ItemCurrencyCounter.Hide(item3);
						}
					}
				}
			}
		...
		void SetupWillGetItemDisplay()
		{
			for (int j = 0; j < _instance.instantiatedItems.Count; j++)
			{
				SavedItemDisplay savedItemDisplay2 = _instance.instantiatedItems[j];
				if (j < 1 && willGetItem != null)
				{
					savedItemDisplay2.gameObject.SetActive(value: true);
					SavedItem item3 = willGetItem;
					int amount2 = 0;
					savedItemDisplay2.Setup(item3, amount2);
				}
				else
				{
					savedItemDisplay2.gameObject.SetActive(value: false);
				}
			}
		}
	}
