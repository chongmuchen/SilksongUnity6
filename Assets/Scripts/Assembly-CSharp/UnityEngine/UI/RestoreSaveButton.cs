...
namespace UnityEngine.UI
{
	public sealed class RestoreSaveButton : MenuButton, ISubmitHandler, IEventSystemHandler, IPointerClickHandler, ISelectHandler
	{
		...
		private const float LOADING_TIMEOUT = 20f;
				...
				if (fetchRequest != null)
				{
					...
					if (isSaveIncompatible && versionDataRequest != null)
					{
						...
						if (versionDataRequest.State == FetchDataRequest.Status.Completed && versionDataRequest.results != null)
						{
							...
							for (int i = 0; i < versionDataRequest.results.Count; i++)
							{
								...
								if (fetchResult.loadedObject != null && !SaveDataUtility.IsVersionIncompatible("1.0.30000", fetchResult.loadedObject.playerData.version, fetchResult.loadedObject.playerData.RevisionBreak))
								{
					...
					if (fetchRequest.results != null)
					{
						...
						for (int j = 0; j < fetchRequest.results.Count; j++)
						{
							...
							if (fetchResult2.loadedObject.saveGameData != null && !SaveDataUtility.IsVersionIncompatible("1.0.30000", fetchResult2.loadedObject.saveGameData.playerData.version, fetchResult2.loadedObject.saveGameData.playerData.RevisionBreak))
							{
