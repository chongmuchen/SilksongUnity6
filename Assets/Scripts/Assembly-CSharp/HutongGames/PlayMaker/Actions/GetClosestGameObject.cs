	...
	public class GetClosestGameObject : FsmStateAction
	{
		...
		private void DoAction()
		{
			...
			if (num != 0)
			{
				...
				matches.Sort(delegate(Match match2, Match match3)
				{
					float num2 = (match2.GameObject.Value ? Vector2.Distance(targetPosition, match2.GameObject.Value.transform.position) : float.MaxValue);
					float value = (match3.GameObject.Value ? Vector2.Distance(targetPosition, match3.GameObject.Value.transform.position) : float.MaxValue);
					return num2.CompareTo(value);
				});
				Match match = matches[0];
				StoreGameObject.Value = match.GameObject.Value;
