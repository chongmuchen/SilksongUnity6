...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class MovePosition2dV2 : ComponentAction<Rigidbody2D>
	{
		...
		private bool DoMovePosition()
		{
			...
			Vector3 vector = ((!targetPosition.IsNone && targetPosition.Value != null) ? ((space != Space.World) ? targetPosition.Value.transform.localPosition : targetPosition.Value.transform.position) : ((!this.vector.IsNone) ? ((Vector3)this.vector.Value) : ((space == Space.World) ? new Vector3(base.rigidbody2d.position.x, base.rigidbody2d.position.y, 0f) : base.cachedTransform.TransformPoint(base.rigidbody.position))));
			if (!x.IsNone)
			{
				vector.x = x.Value;
			}
