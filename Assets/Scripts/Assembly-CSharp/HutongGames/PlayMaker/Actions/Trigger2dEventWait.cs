...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class Trigger2dEventWait : FsmStateAction
	{
		...
		{
			waitFramesCounter = waitFrames.Value;
			if (AddCustomProxy())
			{
				AddCallbackTriggerStay();
			}
			else if (gameObject.OwnerOption != OwnerDefaultOption.UseOwner)
			{
				if (cachedProxy == null)
				{
					GetProxyComponent();
		...
		{
			if (trigger == Trigger2DType.OnTriggerEnter2D && FsmStateAction.TagMatches(collideTag, other) && (other.gameObject.layer == collideLayer.Value || collideLayer.IsNone))
			{
		...
		{
			if (trigger == Trigger2DType.OnTriggerStay2D && FsmStateAction.TagMatches(collideTag, other) && (other.gameObject.layer == collideLayer.Value || collideLayer.IsNone))
			{
				StoreCollisionInfo(other);
				base.Fsm.Event(sendEvent);
			}
		...
		{
			if (trigger == Trigger2DType.OnTriggerExit2D && FsmStateAction.TagMatches(collideTag, other) && (other.gameObject.layer == collideLayer.Value || collideLayer.IsNone))
			{
				StoreCollisionInfo(other);
				base.Fsm.Event(sendEvent);
			}
