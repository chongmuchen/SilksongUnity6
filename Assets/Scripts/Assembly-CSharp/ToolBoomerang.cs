...
public class ToolBoomerang : MonoBehaviour, IHitResponder, ITinkResponder
{
	...
	private void OnDrawGizmosSelected()
	{
		...
		Vector2 vector = (Application.isPlaying ? initialPosition : Vector2.zero);
		...
		{
			Gizmos.DrawWireSphere(SamplePosAtTime(vector, to, num), 0.1f);
		}
		Gizmos.color = Color.red;
		for (float num2 = 1f; num2 <= 2f; num2 += 0.05f)
		{
			Gizmos.DrawWireSphere(SamplePosAtTime(vector, to, num2), 0.1f);
		}
		Vector2 vector2 = SamplePosAtTime(vector, to, flyOutReturnPoint);
