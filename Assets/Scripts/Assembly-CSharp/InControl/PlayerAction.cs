	...
	public class PlayerAction : OneAxisInputControl
	{
		...
		public void AddDefaultBinding(BindingSource binding)
		{
			if (!(binding == null))
			{
				...
				TryAddBinding(defaultBindings, binding);
				if (TryAddBinding(regularBindings, binding) && binding.IsValid)
				{
		...
		private bool TryAddBinding(List<BindingSource> bindingList, BindingSource binding)
		{
			int num = bindingList.IndexOf(binding);
			if (num < 0)
			{
			...
			if (bindingList[num].BoundTo == null)
			{
