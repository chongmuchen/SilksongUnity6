...
public class SimpleQuestsShopOwner : SimpleShopMenuOwner
{
	...
	[SerializeField]
	[PlayerDataField(typeof(string), false)]
	private string lastCompletedQuestNameString;
	...
	protected override List<ISimpleShopItem> GetItems()
	{
		...
		string text = (string.IsNullOrEmpty(lastCompletedQuestNameString) ? null : instance.GetVariable<string>(lastCompletedQuestNameString));
		...
		foreach (ShopItemInfo quest3 in quests)
		{
			...
			else
			{
				...
				if (quest3.IsRepeatable)
				{
					...
					if (flag)
					{
						if (!string.IsNullOrEmpty(text) && quest.name == text)
						{
							shopItemInfo = quest3;
						}
						else
		...
		if (flag)
		{
			if (runningGenericList.Count < 2 && shopItemInfo != null)
			{
				runningGenericList.Add(shopItemInfo);
			}
		...
		runningList.Clear();
		runningGenericList.Clear();
		if (list != null && list.Count >= 2 && currentList.Count < 2)
		{
			list.Clear();
			instance.SetVariable(genericQuestsList, list);
			GetItems();
