...
public class InventoryItemGrid : InventoryItemSelectable, IInventorySelectionParent
{
	...
	public void Setup(List<GridSection> newCollections)
	{
		...
		for (int num = 0; num < newCollections.Count; num++)
		{
			GridSection gridSection2 = newCollections[num];
			GridSection gridSection3 = ((num > 0) ? newCollections[num - 1] : null);
			GridSection gridSection4 = ((num < newCollections.Count - 1) ? newCollections[num + 1] : null);
			...
			{
				int num2 = gridSection3.Items.Count % RowSplit;
				if (num2 == 0)
				{
					num2 = RowSplit;
				}
				upOverrides = gridSection3.Items.Skip(gridSection3.Items.Count - num2).ToList();
			}
			...
			float num3 = ((inventoryItemSelectableDirectional != null) ? (inventoryItemSelectableDirectional.transform.localPosition.y - GridOffset.y + ItemOffset.y) : 0f);
			PositionGridItems(gridSection2.Items, num3 - SectionPadding);
			if ((bool)newCollections[num].Header)
			{
				newCollections[num].Header.SetLocalPositionY(newCollections[num].Items[0].transform.localPosition.y + SectionHeaderHeight);
			}
			for (int num4 = 0; num4 < gridSection2.Items.Count; num4++)
			{
				InventoryItemSelectableDirectional inventoryItemSelectableDirectional2 = gridSection2.Items[num4];
				inventoryItemSelectableDirectional2.Grid = this;
				inventoryItemSelectableDirectional2.GridSectionIndex = num;
				inventoryItemSelectableDirectional2.GridItemIndex = num4;
			}
		}
		if ((bool)scrollView)
		{
