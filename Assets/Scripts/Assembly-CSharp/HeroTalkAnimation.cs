...
public class HeroTalkAnimation : MonoBehaviour
{
	...
	private void InternalEnterConversation(NPCControlBase sourceNpc)
	{
		...
		else if (sourceAnimNpc.HeroAnimation != AnimationTypes.LookForward)
		{
			SkipWaiting();
	...
	{
		TakeAnimationControl();
		tk2dSpriteAnimationClip clip = heroAnim.GetClip("Hurt To Idle");
		animator.Play(clip);
		endHurtAnimWait = new WaitForTk2dAnimatorClipFinish(animator, delegate(tk2dSpriteAnimator anim, tk2dSpriteAnimationClip tk2dSpriteAnimationClip2)
		{
			if (tk2dSpriteAnimationClip2.name == "Hurt To Idle")
			{
				tk2dSpriteAnimationClip clip2 = heroAnim.GetClip("Idle");
				anim.Play(clip2);
			}
			endHurtAnimWait = null;
			if (onEndHurtAnimCompletion != null)
