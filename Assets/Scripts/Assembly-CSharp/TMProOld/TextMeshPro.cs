	...
	public class TextMeshPro : TMP_Text, ILayoutElement
	{
		...
		public void CalculateLayoutInputHorizontal()
		{
			...
			if (m_isCalculateSizeRequired || m_rectTransform.hasChanged)
			{
				...
				m_flexibleWidth = 0f;
				bool flag = false;
				if (m_enableAutoSizing && m_fontSize != m_fontSizeMax)
				{
		...
		protected override void Awake()
		{
			...
			CanvasRenderer canvasRenderer = base.gameObject.GetComponent<CanvasRenderer>();
			if (!canvasRenderer)
			{
		...
		protected override void LoadFontAsset()
		{
			...
			else
			{
				...
				if (m_renderer.sharedMaterial == null || m_renderer.sharedMaterial.mainTexture == null || m_fontAsset.atlas.GetLegacyInstanceId() != m_renderer.sharedMaterial.GetTexture(ShaderUtilities.ID_MainTex).GetLegacyInstanceId())
				{
					m_renderer.sharedMaterial = m_fontAsset.material;
					m_sharedMaterial = m_fontAsset.material;
				}
		...
		{
			if (m_fontMaterial == null || m_fontMaterial.GetLegacyInstanceId() != mat.GetLegacyInstanceId())
			{
		...
		protected override void SetSharedMaterials(Material[] materials)
		{
			...
			for (int i = 0; i < materialCount; i++)
			{
				if (i == 0)
				{
					if (!(materials[i].mainTexture == null) && materials[i].mainTexture.GetLegacyInstanceId() == m_sharedMaterial.mainTexture.GetLegacyInstanceId())
					{
						m_sharedMaterial = (m_fontSharedMaterials[i] = materials[i]);
						m_padding = GetPaddingForMaterial(m_sharedMaterial);
					}
				}
				else if (!(materials[i].mainTexture == null) && materials[i].mainTexture.GetLegacyInstanceId() == m_subTextObjects[i].sharedMaterial.mainTexture.GetLegacyInstanceId() && m_subTextObjects[i].isDefaultMaterial)
				{
		...
		protected override int SetArraySizes(int[] chars)
		{
			...
			if (m_currentMaterial == null)
			{
				...
				if (m_currentFontAsset.material == null)
				{
					return 0;
				}
			...
			for (int i = 0; chars[i] != 0; i++)
			{
				...
				if (!m_currentFontAsset.characterDictionary.TryGetValue(num2, out var value))
				{
					...
					if (value == null)
					{
						int num3 = ((TMP_Settings.missingGlyphCharacter == 0) ? 9633 : TMP_Settings.missingGlyphCharacter);
			...
			for (int m = 0; m < num4; m++)
			{
				if (m > 0)
				{
					...
					if (m_subTextObjects[m].sharedMaterial == null || m_subTextObjects[m].sharedMaterial.GetLegacyInstanceId() != m_materialReferences[m].material.GetLegacyInstanceId())
					{
						bool isDefaultMaterial = m_materialReferences[m].isDefaultMaterial;
						m_subTextObjects[m].isDefaultMaterial = isDefaultMaterial;
						if (!isDefaultMaterial || m_subTextObjects[m].sharedMaterial == null || m_subTextObjects[m].sharedMaterial.mainTexture.GetLegacyInstanceId() != m_materialReferences[m].material.GetTexture(ShaderUtilities.ID_MainTex).GetLegacyInstanceId())
						{
				...
				if (m_textInfo.meshInfo[m].vertices == null || m_textInfo.meshInfo[m].vertices.Length < referenceCount * ((!m_isVolumetricText) ? 4 : 8))
				{
					...
					{
						m_textInfo.meshInfo[m].ResizeMeshInfo((referenceCount > 1024) ? (referenceCount + 256) : Mathf.NextPowerOfTwo(referenceCount), m_isVolumetricText);
					}
				}
				else if (m_textInfo.meshInfo[m].vertices.Length - referenceCount * ((!m_isVolumetricText) ? 4 : 8) > 1024)
				{
					m_textInfo.meshInfo[m].ResizeMeshInfo((referenceCount > 1024) ? (referenceCount + 256) : Mathf.Max(Mathf.NextPowerOfTwo(referenceCount), 256), m_isVolumetricText);
				}
			}
		...
		private static bool IsHangul(uint c)
		{
			if ((c < 4352 || c > 4607) && (c < 43360 || c > 43391) && (c < 55216 || c > 55295) && (c < 12592 || c > 12687) && (c < 65440 || c > 65500))
			{
				if (c >= 44032)
				{
					return c <= 55215;
		...
		protected override void GenerateTextMesh()
		{
			...
			{
				Debug.LogWarning("Can't Generate Mesh! No Font Asset has been assigned to Object ID: " + this.GetLegacyInstanceId());
				return;
			}
			if (m_textInfo != null)
			{
			...
			for (int i = 0; m_char_buffer[i] != 0; i++)
			{
				...
				if (m_textElementType == TMP_TextElementType.Sprite)
				{
					...
					m_currentFontAsset = m_fontAsset;
					float num17 = m_currentFontSize / m_fontAsset.fontInfo.PointSize * m_fontAsset.fontInfo.Scale * (m_isOrthographic ? 1f : 0.1f);
				...
				else if (m_textElementType == TMP_TextElementType.Character)
				{
					...
					m_currentMaterialIndex = m_textInfo.characterInfo[m_characterCount].materialReferenceIndex;
					m_fontScale = m_currentFontSize * num16 / m_currentFontAsset.fontInfo.PointSize * m_currentFontAsset.fontInfo.Scale * (m_isOrthographic ? 1f : 0.1f);
				...
				float num21 = m_currentFontAsset.fontInfo.Ascender * ((m_textElementType == TMP_TextElementType.Character) ? num2 : m_textInfo.characterInfo[m_characterCount].scale) + m_baselineOffset;
				...
				float num22 = m_currentFontAsset.fontInfo.Descender * ((m_textElementType == TMP_TextElementType.Character) ? num2 : m_textInfo.characterInfo[m_characterCount].scale) + m_baselineOffset;
				...
				if (num4 == 9 || !char.IsWhiteSpace((char)num4) || m_textElementType == TMP_TextElementType.Sprite)
				{
					...
					num12 = ((m_width != -1f) ? Mathf.Min(marginWidth + 0.0001f - m_marginLeft - m_marginRight, m_width) : (marginWidth + 0.0001f - m_marginLeft - m_marginRight));
					m_textInfo.lineInfo[m_lineNumber].marginLeft = m_marginLeft;
					if (Mathf.Abs(m_xAdvance) + ((!m_isRightToLeft) ? m_cached_TextElement.xAdvance : 0f) * (1f - m_charWidthAdjDelta) * ((num4 != 173) ? num2 : num18) > num12)
					{
				...
				if (num4 == 9)
				{
					...
					m_xAdvance = ((num32 > m_xAdvance) ? num32 : (m_xAdvance + num31));
				}
				...
				if (num4 == 10 || m_characterCount == totalCharacterCount - 1)
				{
					...
					m_maxDescender = ((m_maxDescender < num35) ? m_maxDescender : num35);
					if (!flag4)
					{
						num14 = m_maxDescender;
					}
				...
				if (m_overflowMode == TextOverflowModes.Page && num4 != 13 && num4 != 10 && m_pageNumber < 16)
				{
					...
					m_textInfo.pageInfo[m_pageNumber].descender = ((num22 < m_textInfo.pageInfo[m_pageNumber].descender) ? num22 : m_textInfo.pageInfo[m_pageNumber].descender);
				...
				if (m_enableWordWrapping || m_overflowMode == TextOverflowModes.Truncate || m_overflowMode == TextOverflowModes.Ellipsis)
				{
					...
					else if (((num4 > 4352 && num4 < 4607) || (num4 > 11904 && num4 < 40959) || (num4 > 43360 && num4 < 43391) || (num4 > 44032 && num4 < 55295) || (num4 > 63744 && num4 < 64255) || (num4 > 65072 && num4 < 65103) || (num4 > 65280 && num4 < 65519)) && !m_isNonBreakingSpace && !IsHangul((uint)num4))
					{
						if (!flag7 && (flag5 || flag6 || (!TMP_Settings.linebreakingRules.leadingCharacters.ContainsKey(num4) && m_characterCount < totalCharacterCount - 1 && !TMP_Settings.linebreakingRules.followingCharacters.ContainsKey(m_textInfo.characterInfo[m_characterCount + 1].character))))
						{
							SaveWordWrappingState(ref m_SavedWordWrapState, i, m_characterCount);
							m_isCharacterWrappingEnabled = false;
							flag5 = false;
			...
			case TextAlignmentOptions.TopJustified:
				vector4 = ((m_overflowMode == TextOverflowModes.Page) ? (textContainerLocalCorners[1] + new Vector3(0f + vector.x, 0f - m_textInfo.pageInfo[num10].ascender - vector.y, 0f)) : (textContainerLocalCorners[1] + new Vector3(0f + vector.x, 0f - m_maxAscender - vector.y, 0f)));
				break;
			case TextAlignmentOptions.Left:
			case TextAlignmentOptions.Center:
			case TextAlignmentOptions.Right:
			case TextAlignmentOptions.Justified:
				vector4 = ((m_overflowMode == TextOverflowModes.Page) ? ((textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + vector.x, 0f - (m_textInfo.pageInfo[num10].ascender + vector.y + m_textInfo.pageInfo[num10].descender - vector.w) / 2f, 0f)) : ((textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + vector.x, 0f - (m_maxAscender + vector.y + num14 - vector.w) / 2f, 0f)));
				break;
			case TextAlignmentOptions.BottomLeft:
			case TextAlignmentOptions.Bottom:
			case TextAlignmentOptions.BottomRight:
			case TextAlignmentOptions.BottomJustified:
				vector4 = ((m_overflowMode == TextOverflowModes.Page) ? (textContainerLocalCorners[0] + new Vector3(0f + vector.x, 0f - m_textInfo.pageInfo[num10].descender + vector.w, 0f)) : (textContainerLocalCorners[0] + new Vector3(0f + vector.x, 0f - num14 + vector.w, 0f)));
				break;
			case TextAlignmentOptions.BaselineLeft:
			case TextAlignmentOptions.Baseline:
			case TextAlignmentOptions.BaselineRight:
			case TextAlignmentOptions.BaselineJustified:
				vector4 = (textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + vector.x, 0f, 0f);
				break;
			case TextAlignmentOptions.MidlineLeft:
			case TextAlignmentOptions.Midline:
			case TextAlignmentOptions.MidlineRight:
			case TextAlignmentOptions.MidlineJustified:
				vector4 = (textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + vector.x, 0f - (m_meshExtents.max.y + vector.y + m_meshExtents.min.y - vector.w) / 2f, 0f);
				break;
			case TextAlignmentOptions.CaplineLeft:
			case TextAlignmentOptions.Capline:
			case TextAlignmentOptions.CaplineRight:
			case TextAlignmentOptions.CaplineJustified:
				vector4 = (textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + vector.x, 0f - (m_maxCapHeight - vector.y - vector.w) / 2f, 0f);
				break;
			}
			...
			for (int j = 0; j < m_characterCount; j++)
			{
				...
				case TextAlignmentOptions.MidlineLeft:
				case TextAlignmentOptions.CaplineLeft:
					vector5 = (m_isRightToLeft ? new Vector3(0f - tMP_LineInfo.maxAdvance, 0f, 0f) : new Vector3(0f + tMP_LineInfo.marginLeft, 0f, 0f));
					break;
				case TextAlignmentOptions.Top:
				case TextAlignmentOptions.Center:
				case TextAlignmentOptions.Bottom:
				...
				case TextAlignmentOptions.CaplineJustified:
				{
					...
					if (char.IsControl(characterInfo[tMP_LineInfo.lastCharacterIndex].character) || lineNumber >= m_lineNumber)
					{
					...
					float num51 = ((tMP_LineInfo.spaceCount > 2) ? m_wordWrappingRatios : 1f);
				...
				else if ((flag8 || (j == 0 && (!char.IsPunctuation(character2) || char.IsWhiteSpace(character2) || j == m_characterCount - 1))) && (j <= 0 || j >= characterInfo.Length - 1 || j >= m_characterCount || (character2 != '\'' && character2 != '’') || !char.IsLetterOrDigit(characterInfo[j - 1].character) || !char.IsLetterOrDigit(characterInfo[j + 1].character)))
				{
		...
		protected override Bounds GetCompoundBounds()
		{
			...
			return new Bounds(size: vector2 - vector, center: vector3);
		}
