...
public class ClockworkHatchling : MonoBehaviour
{
	...
	[SerializeField]
	private Vector2 attackCooldownRange;
	...
	private void Awake()
	{
		...
		EventRegister.GetRegisterGuaranteed(base.gameObject, "CLEAR EFFECTS INSTANT").ReceivedEvent += ClearEffectInstant;
	}
		...
		if ((bool)enemyRange)
		{
			enemyRange.OnTriggerStayed += delegate(Collider2D collision, GameObject _)
			{
				if (canDamage)
				{
					...
					{
						EventRegister.SendEvent("COGFLY ATTACK");
	...
	private void OnEnable()
	{
		...
		instance2.OnBeforeFinishedSceneTransition += OnBeforeFinishedSceneTransition;
		instance2.GameStateChange += GameStateChange;
	...
	private void OnDisable()
	{
		...
		if ((bool)instance)
		{
			instance.OnBeforeFinishedSceneTransition -= OnBeforeFinishedSceneTransition;
			instance.GameStateChange -= GameStateChange;
		}
	...
	private void RecordPoisonStatus()
	{
		if (!hasCheckedPoison)
		{
			hasCheckedPoison = true;
			isPoison = Gameplay.PoisonPouchTool.IsEquipped && !ToolItemManager.IsCustomToolOverride;
		}
	}
	...
	private IEnumerator Spawn()
	{
		...
		if (!quickSpawn)
		{
			...
			if ((bool)body)
			{
				...
				{
					x = num * Mathf.Cos(num2 * (MathF.PI / 180f)),
					y = num * Mathf.Sin(num2 * (MathF.PI / 180f))
				};
	...
	private IEnumerator HitLanded()
	{
		...
		if (hpCurrent <= 0)
		{
			...
			if ((bool)animator)
			{
				...
				{
					animator.Play(clipByName);
					explodeWaitTime = clipByName.Duration;
				}
			}
	...
	private void ClearEffectInstant()
	{
	...
	private void OnBeforeFinishedSceneTransition()
	{
		if (willBreak)
		{
			base.gameObject.Recycle();
	...
	{
		if (willBreak && gameState == GameState.ENTERING_LEVEL)
		{
			base.gameObject.Recycle();
		}
	}
	...
	{
		cooldownTimer = UnityEngine.Random.Range(attackCooldownRange.x, attackCooldownRange.y);
	}
