...
public class CustomSceneManager : MonoBehaviour
{
	...
	private void UpdateAppearanceRegion(bool forceImmediate)
	{
		...
		float newSaturation = AdjustSaturation(saturation);
		bool flag = false;
		bool flag2 = false;
		...
		foreach (SceneAppearanceRegion appearanceRegion in appearanceRegions)
		{
			...
			{
				newSaturation = AdjustSaturation(appearanceRegion.Saturation);
		...
		if (flag2 || didAffectSaturation)
		{
			...
			{
				float saturationA = Mathf.Lerp(startSaturation, newSaturation, time);
				gc.colorCorrectionCurves.saturation = saturationA;
				gc.sceneColorManager.SaturationA = saturationA;
			});
		}
