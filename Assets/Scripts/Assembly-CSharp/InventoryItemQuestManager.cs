...
public class InventoryItemQuestManager : QuestItemManager, IInventoryPaneAvailabilityProvider
{
	...
	protected override void OnItemListSetup()
	{
		...
		{
			BasicQuestBase basicQuestBase = list[num2];
			InventoryItemMainQuest inventoryItemMainQuest = spawnedMainQuestItems[num2];
			...
			{
				InventoryItemMainQuest inventoryItemMainQuest2 = spawnedMainQuestItems[num2 + 1];
		...
		for (int num3 = list.Count; num3 < spawnedMainQuestItems.Count; num3++)
		{
			spawnedMainQuestItems[num3].gameObject.SetActive(value: false);
		}
		if (list.Count <= 0)
		{
