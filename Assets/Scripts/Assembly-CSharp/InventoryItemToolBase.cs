...
public abstract class InventoryItemToolBase : InventoryItemUpdateable
{
	...
	protected void ItemDataUpdated()
	{
		...
		if ((bool)extraDesc)
		{
			extraDesc.ExtraDescPrefab = (((bool)itemData && itemData.IsUnlockedNotHidden) ? itemData.ExtraDescriptionSection : null);
		}
		if ((bool)zapIcon)
		{
			zapIcon.sprite = null;
	...
	public override bool Extra()
	{
		...
		if (itemData.ReplenishUsage == ToolItem.ReplenishUsages.OneForOne)
		{
			holdTimerLeft = 0.3f;
			return true;
		}
		if (!itemData.DisplayTogglePrompt)
		{
			return base.Extra();
		}
		return DoExtraPress();
