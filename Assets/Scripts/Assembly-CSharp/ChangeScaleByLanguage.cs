...
public sealed class ChangeScaleByLanguage : ChangeByLanguageOld<ChangeScaleByLanguage.ScaleOverride>
{
	...
	protected override void ApplySetting(ScaleOverride setting)
	{
		...
		void ApplySetting(HandHeldOverrides handHeldOverrides)
		{
			if (handHeldOverrides.xScale.IsEnabled)
			{
				scale.x = handHeldOverrides.xScale.Value;
			}
			if (handHeldOverrides.yScale.IsEnabled)
			{
				scale.y = handHeldOverrides.yScale.Value;
			}
			if (handHeldOverrides.zScale.IsEnabled)
			{
				scale.z = handHeldOverrides.zScale.Value;
			}
		}
	}
}
