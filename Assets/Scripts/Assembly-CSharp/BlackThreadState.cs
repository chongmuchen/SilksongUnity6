...
public class BlackThreadState : MonoBehaviour, Recoil.IRecoilMultiplier, IInitialisable
{
	...
	public bool OnAwake()
	{
		...
		for (int num = 0; num < tk2dSpriteRenderers.Length; num++)
		{
			tk2dSpriteRenderers[num] = tk2dSprites[num].GetComponent<MeshRenderer>();
		}
	...
	private void FirstThreadedSetUp()
	{
		...
		{
			array2[num].material.EnableKeyword("BLACKTHREAD");
		}
		MeshRenderer[] array3 = extraMeshRenderers;
		for (int num = 0; num < array3.Length; num++)
		{
			array3[num].material.EnableKeyword("BLACKTHREAD");
		}
	...
	public void ResetThreaded()
	{
		...
		if (IsVisiblyThreaded)
		{
			...
			{
				tk2dSprites[j].color = initialColors[j];
			}
	...
	private IEnumerator ThreadAttackTest()
	{
		...
		while (true)
		{
			...
			if (timeOutLeft > 0f)
			{
				...
				for (int num = 0; num < tk2dSpriteRenderers.Length; num++)
				{
					startColors[num] = tk2dSprites[num].color;
				}
				for (float elapsed = 0f; elapsed <= duration; elapsed += Time.deltaTime)
				{
					float t = curve.Evaluate(elapsed / duration);
					for (int num2 = 0; num2 < tk2dSpriteRenderers.Length; num2++)
					{
						tk2dSprites[num2].color = Color.Lerp(startColors[num2], Color.black, t);
					}
					yield return null;
				}
				if (hasRb2d)
				...
				for (float elapsed = 0f; elapsed <= duration; elapsed += Time.deltaTime)
				{
					...
					for (int num3 = 0; num3 < tk2dSpriteRenderers.Length; num3++)
					{
						tk2dSprites[num3].color = Color.Lerp(Color.black, initialColors[num3], t2);
					}
					yield return null;
				}
				StartPulseRoutine();
	...
	{
		if (!currentAttack || !spawnedAttackObjs.TryGetValue(currentAttack, out var value) || !value || ((bool)healthManager && healthManager.isDead))
		{
			return;
		}
