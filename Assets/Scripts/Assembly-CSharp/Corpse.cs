...
public class Corpse : MonoBehaviour, IInitialisable, BlackThreadState.IBlackThreadStateReceiver
{
	...
	private void OnCorpseBegin(Transform trans)
	{
	...
	protected virtual void Smash()
	{
		if (!hitAcid)
		{
			BloodSpawner.SpawnBlood(base.transform.position, 6, 8, 10f, 20f, 75f, 105f);
		}
		...
		if (bigBreaker)
		{
			if (!hitAcid)
			{
				BloodSpawner.SpawnBlood(base.transform.position, 30, 30, 20f, 30f, 80f, 100f);
			}
