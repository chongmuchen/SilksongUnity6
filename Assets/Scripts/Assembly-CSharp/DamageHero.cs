...
public class DamageHero : MonoBehaviour, IInitialisable
{
	...
	private static HashSet<DamageHero> toggledDamagedHeroes = new HashSet<DamageHero>();
	...
	private void OnDestroy()
	{
		...
		toggledDamagedHeroes.Remove(this);
	}
	...
	public static void ToggleDamaged(DamageHero damageHero, bool enabled)
	{
		if (enabled)
		{
			damageHero.enabled = true;
			toggledDamagedHeroes.Remove(damageHero);
		}
		else
		{
			damageHero.enabled = false;
			toggledDamagedHeroes.Add(damageHero);
		}
	}

	public static void ResetRecordedDamagers()
	{
		foreach (DamageHero toggledDamagedHero in toggledDamagedHeroes)
		{
			toggledDamagedHero.enabled = true;
		}
		toggledDamagedHeroes.Clear();
	}
