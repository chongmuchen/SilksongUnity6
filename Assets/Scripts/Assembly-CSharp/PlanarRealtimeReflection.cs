...
public class PlanarRealtimeReflection : MonoBehaviour
{
	...
	private void CreateSurfaceObjects(Camera currentCamera, out Camera reflectionCamera)
	{
		...
		if (!m_ReflectionTexture || m_OldReflectionTextureSize != m_TextureResolution)
		{
			...
			m_ReflectionTexture.name = "__SurfaceReflection" + this.GetLegacyInstanceId();
		...
		if (!reflectionCamera)
		{
			GameObject gameObject = new GameObject("Surface Refl Camera id" + this.GetLegacyInstanceId() + " for " + currentCamera.GetLegacyInstanceId(), typeof(Camera), typeof(Skybox));
