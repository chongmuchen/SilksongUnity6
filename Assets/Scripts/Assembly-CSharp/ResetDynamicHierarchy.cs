...
public class ResetDynamicHierarchy : MonoBehaviour, AntRegion.INotify
{
	...
	public void Disconnect(GameObject target, bool recursive)
	{
		if (!(target == null))
		{
			Disconnect(target.transform, recursive);
		}
	}
	...
	private static void CaptureStatesRecursive(Transform target, List<State> addState)
	{
		...
		addState.Add(new State
		{
			...
			BehaviourEnabledStates = (from b in target.GetComponents<MonoBehaviour>()
				select new BehaviourActivation
				{
					Behaviour = b,
					Enabled = b.enabled
				}).ToArray(),
			ColliderEnabledStates = (from c in target.GetComponents<Collider2D>()
				select new ColliderActivation
				{
					Collider = c,
					Enabled = c.enabled
				}).ToArray()
