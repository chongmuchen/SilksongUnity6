...
public class InventoryPaneInput : MonoBehaviour
{
	...
	private void PressCancel()
	{
		...
		actionCooldown = 0.25f;
		isRepeatingDirection = false;
		VibrationManager.PlayVibrationClipOneShot(menuCancelVibration);
	}

	private void PressDirection(InventoryPaneBase.InputEventType direction)
	{
		if ((allowHorizontalSelection || (direction != InventoryPaneBase.InputEventType.Left && direction != InventoryPaneBase.InputEventType.Right)) && (allowVerticalSelection || (direction != InventoryPaneBase.InputEventType.Up && direction != InventoryPaneBase.InputEventType.Down)))
		{
			if (allowRepeat)
			{
	...
	private void PressSubmit()
	{
		...
		isRepeatingDirection = false;
		VibrationManager.PlayVibrationClipOneShot(menuSubmitVibration);
