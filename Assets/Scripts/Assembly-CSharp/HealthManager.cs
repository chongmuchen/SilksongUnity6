...
public class HealthManager : MonoBehaviour, IHitResponder, ITagDamageTakerOwner, IInitialisable
{
	...
	private class StealLagHit : LagHitOptions
	{
		...
		private readonly bool doSteal;

		public StealLagHit(HealthManager healthManager, bool doSteal)
		{
			this.healthManager = healthManager;
			this.doSteal = doSteal;
			UseNailDamage = true;
		...
		public override void OnEnd(Transform effectsPoint, Vector3 effectOrigin, HitInstance hitInstance)
		{
			...
			int stolenShards;
			int flingShards;
			if ((bool)healthManager && doSteal)
			{
				healthManager.StealCurrency(out stolenGeo, out flingGeo, out stolenShards, out flingShards);
			}
			else
			{
				flingShards = 0;
	...
	private const float rapidBombTime = 2f;

	private float rapidBombTimer;

	private int rapidBombCount;

	private const float rapidStormTime = 0.6f;

	private float rapidStormTimer;

	private int rapidStormCount;
	...
	protected void Update()
	{
		...
		if (rapidStormTimer > 0f)
		{
			rapidStormTimer -= Time.deltaTime;
			if (rapidStormTimer <= 0f)
			{
				rapidStormCount = 0;
			}
	...
	{
		int level = (hitInstance.IsUsingNeedleDamageMult ? PlayerData.instance.nailUpgrades : ((!hitInstance.RepresentingTool) ? (hitInstance.DamageScalingLevel - 1) : PlayerData.instance.ToolKitUpgrades));
	...
	private void TakeDamage(HitInstance hitInstance)
	{
		...
		if ((hitInstance.SpecialType & SpecialTypes.RapidStorm) != SpecialTypes.None)
		{
			rapidStormTimer = 0.6f;
			rapidStormCount++;
			if (rapidStormCount > 4)
			{
				if (rapidStormCount == 5)
				{
		...
		case AttackTypes.Spell:
			if ((hitInstance.AttackType == AttackTypes.Nail || (hitInstance.AttackType == AttackTypes.Heavy && hitInstance.IsNailTag)) && enemyType != EnemyTypes.Shade && enemyType != EnemyTypes.Armoured && !DoNotGiveSilk)
			{
				if (!flag)
				{
	...
	{
		hp = Mathf.Max(hp - damageAmount, -1000);
		if (hp <= 0)
		{
	...
	{
		hitInstance = ApplyDamageScaling(hitInstance);
		hp = Mathf.Max(hp - hitInstance.DamageDealt, -1000);
		if (hp <= 0)
		{
	...
	{
		if (!disallowDropFling && GameManager.instance.IsMemoryScene())
		{
	...
	public void Die(float? attackDirection, AttackTypes attackType, NailElements nailElement, GameObject damageSource, bool ignoreEvasion = false, float corpseFlingMultiplier = 1f, bool overrideSpecialDeath = false, bool disallowDropFling = false)
	{
		...
		if (!disallowDropFling && attackType != AttackTypes.RuinsWater && !GameManager.instance.IsMemoryScene())
		{
			int smallGeoExtra = 0;
		...
		if (!overrideSpecialDeath)
		{
			...
			FSMUtility.SendEventToGameObject(go, "FATAL DAMAGE");
		}
		isDead = true;
		if (damageHero != null)
		{
	...
	private void SpawnCurrency(Transform spawnPoint, float dropFlingSpeedMin, float dropFlingSpeedMax, float dropFlingAngleMin, float dropFlingAngleMax, int smallGeoCount, int mediumGeoCount, int largeGeoCount, int largeSmoothGeoCount, bool shouldGeoFlash, int shellShardCount, bool shouldShardsFlash)
	{
		if (smallGeoCount > 0)
		{
			...
			}, spawnPoint, effectOrigin, spawnedFlingTracker);
			if (shouldGeoFlash)
			{
	...
	public bool IsBlockingByDirection(int cardinalDirection, AttackTypes attackType, SpecialTypes specialType = SpecialTypes.None)
	{
		...
		if ((piercable || invincibleFromDirection != 0) && (attackType == AttackTypes.Explosion || attackType == AttackTypes.Lightning || (specialType & SpecialTypes.Piercer) != SpecialTypes.None))
		{
			return false;
		}
	...
	public void DoLagHits(LagHitOptions options, HitInstance hitInstance)
	{
		...
		OverrideBool nonLethalOverride = options.NonLethalOverride;
		if (nonLethalOverride != null && nonLethalOverride.IsEnabled)
		{
			hitInstance.NonLethal = options.NonLethalOverride.Value;
		}
	...
	public void DoStealHit()
	{
		...
		{
			DoLagHits(new StealLagHit(this, !GameManager.instance.IsMemoryScene()), lastHitInstance);
		}
	}

	public void StealCurrency(out int stolenGeo, out int flingGeo, out int stolenShards, out int flingShards)
	{
		stolenGeo = 0;
		flingGeo = 0;
		stolenShards = 0;
