...
public class MenuButtonList : MonoBehaviour
{
	...
	private class Entry
	{
		...
		[Space]
		[SerializeField]
		private MenuOptionHorizontal otherRequired;
		...
		public MenuOptionHorizontal OtherRequired => otherRequired;
	...
	public void SetupActive()
	{
		...
		foreach (Entry entry in array)
		{
			...
			if (condition == null || condition.IsFulfilledAllComponents())
			{
				...
				if ((bool)entry.OtherRequired)
				{
					bool flag2 = entry.OtherRequired.selectedOptionIndex != 0;
					entry.Group.interactable = flag2;
					entry.Group.blocksRaycasts = flag2;
					entry.Group.alpha = (flag2 ? 1f : 0.5f);
					if (flag2)
					{
	...
	private IEnumerator SelectDelayed(Selectable selectable)
	{
		...
		{
			if (animator.HasParameter(_hide))
			{
				animator.ResetTrigger(_hide);
			}
			if (animator.HasParameter(_show))
			{
