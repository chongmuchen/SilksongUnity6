...
public class Recoil : MonoBehaviour
{
	...
	public void RecoilByDirection(int attackDirection, float attackMagnitude)
	{
		...
		{
			if ((attackDirection == 1 && IsUpBlocked) || (attackDirection == 3 && IsDownBlocked) || (attackDirection == 2 && IsLeftBlocked) || (attackDirection == 0 && IsRightBlocked))
			{
				return;
			}
			...
			if (!bodyCollider)
			{
				Debug.LogError("Couldn't get collider for recoil, trying children...");
				bodyCollider = GetComponentInChildren<Collider2D>();
				if (!bodyCollider)
				{
					Debug.LogError("Still couldn't get collider for recoil! Will not recoil.");
					return;
				}
			}
