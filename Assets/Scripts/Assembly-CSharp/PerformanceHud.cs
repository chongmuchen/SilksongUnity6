...
public class PerformanceHud : MonoBehaviour, IOnGUI
{
	...
	private static void LabelWithShadowRight(GUIContent content, ref int lineIndex)
	{
		...
		LabelWithShadow(new Rect((float)Screen.width - vector.x - ScreenEdgePadding, (float)(Screen.height - LineHeight * lineIndex) - ScreenEdgePadding, vector.x + 10f, LineHeight), content);
	}
