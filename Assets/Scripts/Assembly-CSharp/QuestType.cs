...
public class QuestType : ScriptableObject
{
	...
	private string recordQuestNameOnComplete;
	...
	public void OnQuestCompleted(FullQuestBase quest)
	{
		...
		{
			instance.GetVariable<List<string>>(removeQuestFromListOnComplete)?.Remove(quest.name);
		}
		if (!string.IsNullOrEmpty(recordQuestNameOnComplete))
		{
			instance.SetVariable(recordQuestNameOnComplete, quest.name);
		}
	}
}
