...
public static class TimeManager
{
	...
	private static void UpdateTimeScale()
	{
		...
		{
			foreach (TimeControlInstance timeControlInstance in _timeControlInstances)
			{
		...
		if (num > 1f && !CheatManager.IsCheatsEnabled)
		{
			num = 1f;
		}
		float num2 = Mathf.Max(0f, num);
		if (!Mathf.Approximately(Time.timeScale, num2))
		{
			Time.timeScale = num2;
