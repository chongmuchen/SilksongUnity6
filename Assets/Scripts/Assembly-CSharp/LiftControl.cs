...
public class LiftControl : MonoBehaviour, HeroPlatformStick.IMoveHooks
{
	...
	private void OnDrawGizmosSelected()
	{
		...
		foreach (LiftStop liftStop in array)
		{
			...
			Vector3 vector = position.Where(null, y);
	...
	private void Awake()
	{
		...
		if ((bool)persistentStopState)
		{
			...
			{
				int num3 = stops.Length;
				if (num3 != 0)
				{
					if (!overridingDefaultStop)
					{
						currentStop = Mathf.Clamp(value, 0, num3 - 1);
					}
					SetInitialPos();
				}
			};
		...
		for (int num2 = 0; num2 < stops.Length; num2++)
		{
			LiftStop stop = stops[num2];
	...
	public void MoveToStopNotInside(int stopIndex)
	{
		MoveToStop(stopIndex, isControllingCamera: false);
	}
