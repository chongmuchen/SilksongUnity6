...
public class ChangeFontByLanguage : MonoBehaviour
{
	...
	private class FontScaleLang
	{
		...
		public float GetFontScale(string lang, float defaultScale)
		{
			...
			case "ZH_TW":
				if (!fontSizeZH_TW.HasValue)
				{
					return defaultScale;
				}
				return fontSizeZH_TW.Value;
			case "KO":
	...
	public TMP_FontAsset fontZH_TW;

	public bool copyMaterialSettingsZH_TW = true;
	...
	public void SetFont()
	{
		...
		case LanguageCode.ZH_TW:
			tmpro.fontSize = GetFontScale(languageCode);
			SetFont(fontZH_TW, copyMaterialSettingsZH_TW);
			break;
		case LanguageCode.KO:
			tmpro.fontSize = GetFontScale(languageCode);
