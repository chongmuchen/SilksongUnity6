...
public sealed class RangeAttackGroup : MonoBehaviour
{
	...
	{
		if (!anyAttackerActive || collision.gameObject.layer != 20)
		{
			return;
		}
		HeroController componentInParent = collision.gameObject.GetComponentInParent<HeroController>();
		if (!componentInParent || componentInParent.CanTakeDamageIgnoreInvul())
		{
			if ((bool)sinkTarget)
			{
			...
			componentInParent.CancelDownspike();
		}
	}
	...
	private bool CanDamagerShred(DamageEnemies otherDamager)
	{
		...
		if ((bool)representingTool && (representingTool.DamageFlags & ToolDamageFlags.Shredding) != ToolDamageFlags.None)
		{
			return true;
		}
		return false;
