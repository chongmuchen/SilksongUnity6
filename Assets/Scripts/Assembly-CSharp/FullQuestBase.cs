...
public abstract class FullQuestBase : BasicQuestBase, IQuestWithCompletion
{
	...
	[SerializeField]
	private PersistentBoolTest[] persistentBoolTests;
	...
	public int RewardCount
	{
		get
		{
			if (!PlayerData.instance.blackThreadWorld || rewardCountAct3 <= 0)
			{
				return rewardCount;
			}
			return rewardCountAct3;
		}
	}
	...
	public override bool IsAvailable
	{
		get
		{
			...
			if (persistentBoolTests != null)
			{
				PersistentBoolTest[] array4 = persistentBoolTests;
				for (int i = 0; i < array4.Length; i++)
				{
					if (!array4[i].IsFulfilled)
					{
						return false;
					}
				}
	...
	protected virtual void ShowQuestAccepted(Action afterPrompt, bool hasPreviousQuest)
	{
		if (hasPreviousQuest)
		{
			...
			}, null, forceReplacingEffect: true);
