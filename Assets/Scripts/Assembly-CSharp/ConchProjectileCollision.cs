...
public sealed class ConchProjectileCollision : MonoBehaviour
{
	...
	private void CheckCollision(bool checkHit = false)
	{
		...
		void CheckBottom()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				RaycastHit2D raycastHit2D = Helper.Raycast2D(bottomRays[i], -Vector2.up, 0.15f, layerMask);
		...
		void CheckLeft()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				RaycastHit2D raycastHit2D = Helper.Raycast2D(leftRays[i], -Vector2.right, 0.15f, layerMask);
				Collider2D collider = raycastHit2D.collider;
				if (collider != null && (!(previousNormal == raycastHit2D.normal) || !(collider == previousCollider)))
				{
		...
		void CheckRight()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				RaycastHit2D raycastHit2D = Helper.Raycast2D(rightRays[i], Vector2.right, 0.15f, layerMask);
				Collider2D collider = raycastHit2D.collider;
				if (collider != null && (!(previousNormal == raycastHit2D.normal) || !(collider == previousCollider)))
				{
