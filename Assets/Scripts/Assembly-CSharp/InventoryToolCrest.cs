...
public class InventoryToolCrest : InventoryItemSelectableDirectional
{
	...
	public void Setup(ToolCrest newCrestData)
	{
		...
		if ((bool)newCrestData)
		{
			...
			for (int num3 = 0; num3 < newCrestData.Slots.Length; num3++)
			{
				ToolCrest.SlotInfo item = newCrestData.Slots[num3];
				...
				inventoryToolCrestSlot3.SetCrestInfo(this, num3);
		...
		for (int num4 = 0; num4 < activeSlots.Count; num4++)
		{
			InventoryToolCrestSlot inventoryToolCrestSlot4 = activeSlots[num4];
			ToolCrest.SlotInfo slotInfo = activeSlotsData[num4];
