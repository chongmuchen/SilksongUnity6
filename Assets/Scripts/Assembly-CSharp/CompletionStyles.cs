...
public class CompletionStyles : MonoBehaviour
{
	...
	private void UpdateStyle()
	{
		...
		string text = Platform.Current.RoamingSharedData.GetString("unlockedMenuStyle", null);
		for (int j = 0; j < Styles.Length; j++)
		{
			CompletionStyle completionStyle2 = Styles[j];
			if (string.IsNullOrEmpty(completionStyle2.ReadMenuStyleKey) || !(text != completionStyle2.ReadMenuStyleKey))
			{
				SetStyle(j);
				break;
			}
