...
public class BouncePodSequence : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		for (int j = 0; j < sequences.Count; j++)
		{
			...
			for (int k = 0; k < list.Count; k++)
			{
				...
				bouncePod.BounceHit += value;
				bouncePod.InertHit += value;
			}
		}
		if ((bool)persistent)
		{
			persistent.OnGetSaveState += delegate(out int reference)
			{
				reference = currentSequenceIndex;
			};
			persistent.OnSetSaveState += delegate(int num2)
			{
				currentSequenceIndex = num2;
	...
	private void UpdateProgressIndicator(bool isInstant)
	{
		...
		float t = (float)currentSequenceIndex / (float)sequences.Count;
		targetIndicatorRotation = Quaternion.Euler(0f, 0f, progressIndicatorRotationExtents.GetLerpedValue(t));
		...
		SetCogsRotating(value: true);
		indicatorAnimationRoutine = this.StartTimerRoutine(progressIndicatorRotateDelay, progressIndicatorRotateDuration, delegate(float num)
		{
			num = progressIndicatorRotateCurve.Evaluate(num);
			progressIndicator.localRotation = Quaternion.LerpUnclamped(currentIndicatorRotation, targetIndicatorRotation, num);
		}, delegate
		{
