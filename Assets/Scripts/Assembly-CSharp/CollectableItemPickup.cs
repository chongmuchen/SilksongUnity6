...
public class CollectableItemPickup : MonoBehaviour
{
	...
	private bool activatedRead;

	private bool activatedSave;
	...
	private void Update()
	{
		...
		if (!IsWaitingForStoppedMoving || activatedRead)
		{
			return;
		}
	...
	private void Setup()
	{
		CheckActivation();
		ResetPickupDelay();
		if (!activatedRead && (bool)spriteRenderer)
		{
	...
	{
		if (activatedRead || Time.timeAsDouble < canPickupTime)
		{
	...
	private void CheckActivation()
	{
		...
		{
			activatedRead = (bool)item && !item.CanGetMore();
		}
		if (activatedRead)
		{
			if (OnPickedUp != null)
