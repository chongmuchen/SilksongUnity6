...
public class QuestItemDescription : MonoBehaviour
{
	...
	public void SetDisplay(BasicQuestBase quest)
	{
		...
		case FullQuestBase.DescCounterTypes.Icons:
		{
			...
			else
			{
				rangeDisplay.SetFilledOverride(delegate(int index)
				{
					...
					(FullQuestBase.QuestTarget, int) tuple2 = targetsList[index2];
					return tuple2.Item2 >= tuple2.Item1.Count;
				});
			}
			rangeDisplay.SetItemSprites(delegate(int index)
			{
				int index2 = FindTargetIndex(index);
				return fullQuest.GetCounterSpriteOverride(targetsList[index2].target, index);
		...
		case FullQuestBase.DescCounterTypes.Text:
		{
			...
			for (int num5 = 0; num5 < targetsList.Count; num5++)
			{
				(FullQuestBase.QuestTarget, int) tuple = targetsList[num5];
				QuestItemDescriptionText questItemDescriptionText = spawnedTextDisplays[num5];
				questItemDescriptionText.gameObject.SetActive(value: true);
				questItemDescriptionText.SetDisplay(fullQuest, tuple.Item1, tuple.Item2);
			}
