...
public class InventoryCursor : MonoBehaviour
{
	...
	private IEnumerator MoveTo(Transform target, Vector2 glowScale, bool doAnim)
	{
		...
		void SetPositions(float t)
		{
			...
			if ((bool)topRight)
			{
				topRight.SetLocalPosition2D(Vector2.Lerp(topRightInitialPos, boxOffset + vector, t));
			}
			vector.x *= -1f;
			if ((bool)topLeft)
			{
				topLeft.SetLocalPosition2D(Vector2.Lerp(topLeftInitialPos, boxOffset + vector, t));
			}
			vector.y *= -1f;
			if ((bool)bottomLeft)
			{
				bottomLeft.SetLocalPosition2D(Vector2.Lerp(bottomLeftInitialPos, boxOffset + vector, t));
			}
			vector.x *= -1f;
			if ((bool)bottomRight)
			{
				bottomRight.SetLocalPosition2D(Vector2.Lerp(bottomRightInitialPos, boxOffset + vector, t));
			}
			if ((bool)back)
			{
				back.SetLocalPosition2D(Vector2.Lerp(backInitialPos, boxOffset, t));
				back.localScale = Vector3.Lerp(backInitialScale, boxScale, t);
			}
			if ((bool)backGlow)
			{
				backGlow.color = Color.Lerp(startColor, newColor, t);
			}
			if ((bool)group)
			{
				group.AlphaSelf = Mathf.Lerp(startGroupAlpha, targetGroupAlpha, t);
			}
		}
	}
}
