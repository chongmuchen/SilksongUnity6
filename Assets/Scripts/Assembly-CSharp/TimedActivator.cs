...
public class TimedActivator : MonoBehaviour
{
	private class Activator
	{
		...
		public int CurrentActivationID;
	...
	private void ProcessTrackerOnActivatingComplete(List<Tracker> trackerList, float deltaTime)
	{
		...
		for (int i = 0; i < trackerList.Count; i++)
		{
			...
			activator.CurrentActivationID = value.ActivationID;
		}
		if (num < trackerList.Count)
		{
			trackerList.RemoveRange(num, trackerList.Count - num);
	...
	private void ProcessTrackerOnWarningComplete(List<Tracker> trackerList, float deltaTime)
	{
		...
		for (int i = 0; i < trackerList.Count; i++)
		{
			...
			Activator activator = value.Activator;
			if (activator.lastState != LastActivatorState.Deactivate && activator.lastState != LastActivatorState.Warning && (value.ActivationID == activator.CurrentActivationID || value.ActivationID == activationID))
			{
	...
	private void ProcessTrackerOnDeactivatingComplete(List<Tracker> trackerList, float deltaTime)
	{
		...
		for (int i = 0; i < trackerList.Count; i++)
		{
			...
			Activator activator = value.Activator;
			if (activator.lastState != LastActivatorState.NotActive && activator.lastState != LastActivatorState.Deactivate && (value.ActivationID == activator.CurrentActivationID || value.ActivationID == activationID))
			{
	...
	private void StartedActivating(bool isFromHit)
	{
		...
		foreach (Activator allActivator in allActivators)
		{
			if (allActivator.Activate.IsActive && allActivator.lastState != LastActivatorState.NotActive && allActivator.lastState != LastActivatorState.Deactivate)
			{
