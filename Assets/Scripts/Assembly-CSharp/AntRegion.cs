	...
	private void OnDrawGizmos()
	{
		if ((bool)antLeftPoint && (bool)antRightPoint)
		{
			...
			if (Mathf.Abs(vector.x) > Mathf.Abs(vector.y))
			{
				...
				a = localPosition.y;
				Gizmos.DrawLine(vector2, original.Where(null, a));
				Vector3 vector3 = localPosition2;
				Vector3 original2 = localPosition;
				a = localPosition2.y;
				Gizmos.DrawLine(vector3, original2.Where(null, a));
			}
			else
			{
				Gizmos.DrawLine(localPosition, localPosition2.Where(localPosition.x));
				Gizmos.DrawLine(localPosition2, localPosition.Where(localPosition2.x));
			}
		}
	}
