...
namespace TMProOld
{
	public static class TMP_MaterialManager
	{
		...
		public static Material GetStencilMaterial(Material baseMaterial, int stencilID)
		{
			...
			int instanceID = baseMaterial.GetLegacyInstanceId();
			for (int i = 0; i < m_materialList.Count; i++)
			{
				if (m_materialList[i].baseMaterial.GetLegacyInstanceId() == instanceID && m_materialList[i].stencilID == stencilID)
				{
		...
		{
			int instanceID = stencilMaterial.GetLegacyInstanceId();
			for (int i = 0; i < m_materialList.Count; i++)
			{
				if (m_materialList[i].stencilMaterial.GetLegacyInstanceId() == instanceID)
				{
		...
		public static void ReleaseBaseMaterial(Material baseMaterial)
		{
			...
			{
				Debug.Log("Removed last reference to " + m_materialList[num].stencilMaterial.name + " with ID " + m_materialList[num].stencilMaterial.GetLegacyInstanceId());
		...
		{
			int instanceID = sourceMaterial.GetLegacyInstanceId();
			Texture texture = targetMaterial.GetTexture(ShaderUtilities.ID_MainTex);
			int instanceID2 = texture.GetLegacyInstanceId();
			...
			m_fallbackMaterialLookup.Add(material.GetLegacyInstanceId(), num);
			return material;
		}
		...
		public static void AddFallbackMaterialReference(Material targetMaterial)
		{
			...
			{
				int instanceID = targetMaterial.GetLegacyInstanceId();
		...
		public static void CleanupFallbackMaterials()
		{
			for (int i = 0; i < m_fallbackCleanupList.Count; i++)
			{
				...
				if (m_fallbackMaterials.TryGetValue(key, out var value) && value.count < 1)
				{
					...
					m_fallbackMaterialLookup.Remove(fallbackMaterial.GetLegacyInstanceId());
		...
		public static void ReleaseFallbackMaterial(Material fallackMaterial)
		{
			...
			int instanceID = fallackMaterial.GetLegacyInstanceId();
			if (m_fallbackMaterialLookup.TryGetValue(instanceID, out var value) && m_fallbackMaterials.TryGetValue(value, out var value2))
			{
				if (value2.count > 1)
				{
