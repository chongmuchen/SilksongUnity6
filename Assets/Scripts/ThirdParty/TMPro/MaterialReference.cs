...
namespace TMProOld
{
	public struct MaterialReference
	{
		...
		public MaterialReference(int index, TMP_FontAsset fontAsset, TMP_SpriteAsset spriteAsset, Material material, float padding)
		{
			...
			isDefaultMaterial = ((material.GetLegacyInstanceId() == fontAsset.material.GetLegacyInstanceId()) ? true : false);
		...
		{
			int instanceID = fontAsset.GetLegacyInstanceId();
			for (int i = 0; i < materialReferences.Length && materialReferences[i].fontAsset != null; i++)
			{
				if (materialReferences[i].fontAsset.GetLegacyInstanceId() == instanceID)
				{
					return true;
				}
			}
		...
		{
			int instanceID = material.GetLegacyInstanceId();
			int value = 0;
			...
			materialReferences[value].isDefaultMaterial = ((instanceID == fontAsset.material.GetLegacyInstanceId()) ? true : false);
