	...
	public class Mask : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			else
			{
				...
				{
					if (item2.shader.ToString() == _maskedSpriteWorldCoordsShader.ToString() && item2.shader.GetLegacyInstanceId() != _maskedSpriteWorldCoordsShader.GetLegacyInstanceId())
					{
						_maskedSpriteWorldCoordsShader = null;
					}
					if (item2.shader.ToString() == _maskedUnlitWorldCoordsShader.ToString() && item2.shader.GetLegacyInstanceId() != _maskedUnlitWorldCoordsShader.GetLegacyInstanceId())
					{
		...
		public void SetMaskBoolValueInMaterial(string variable, bool value, Material material)
		{
			if (material.shader == _maskedSpriteWorldCoordsShader || material.shader == _maskedUnlitWorldCoordsShader)
			{
				...
				if (value)
				{
					material.EnableKeyword(variable.ToUpper(CultureInfo.InvariantCulture) + "_ON");
				}
				else
				{
					material.DisableKeyword(variable.ToUpper(CultureInfo.InvariantCulture) + "_ON");
				}
			}
		}
