...
namespace InControl
{
	public abstract class SingletonMonoBehavior<TComponent> : MonoBehaviour where TComponent : MonoBehaviour
	{
		...
		public static TComponent Instance
		{
			get
			{
				lock (lockObject)
				{
					...
					hasInstance = true;
					instanceId = instance.GetLegacyInstanceId();
					return instance;
				}
			}
		}
		...
		protected bool EnforceSingleton
		{
			get
			{
				if (this.GetLegacyInstanceId() == Instance.GetLegacyInstanceId())
				{
					return false;
				}
		...
		protected bool IsTheSingleton
		{
			get
			{
				lock (lockObject)
				{
					return this.GetLegacyInstanceId() == instanceId;
				}
			}
		}
		...
		protected virtual void Awake()
		{
			...
			if (this.GetLegacyInstanceId() != instanceId)
			{
				base.enabled = false;
			}
			foreach (TComponent item in from o in FindInstances()
				where o.GetLegacyInstanceId() != instanceId
				select o)
			{
				item.enabled = false;
			}
