...
namespace UnityEngine.UI
{
	...
	public class SaveSlotButton : MenuButton, ISelectHandler, IEventSystemHandler, IDeselectHandler, ISubmitHandler, IPointerClickHandler
	{
		...
		private class PreloadOperation
		{
			...
			{
				if (State != PreloadState.NotStarted)
				{
					return;
				}
		...
		{
			if (!isReload && saveFileState != SaveFileStates.NotStarted && saveFileState != SaveFileStates.Corrupted)
			{
				return;
			}
		...
		{
			return SaveDataUtility.IsVersionIncompatible("1.0.30000", fileVersionText, fileRevisionBreak);
		}
		...
		private IEnumerator FadeUpSlotInfoUninfected()
		{
			...
			{
				activeSaveSlot.alpha = alpha;
				slotNumberText.alpha = alpha;
			}
		}
		...
		private IEnumerator AnimateToSlotState(SlotState nextState, bool updateBlackThread = true)
		{
			...
			if (nextState != SlotState.ClearPrompt && nextState != SlotState.ClearConfirm && nextState != SlotState.Hidden)
			{
		...
		private IEnumerator ValidateDeselect()
		{
			...
			prevSelectedObject = currentCurrentSelectedGameObject;
			yield return new WaitForEndOfFrame();
			currentCurrentSelectedGameObject = EventSystem.current.currentSelectedGameObject;
			if (!(currentCurrentSelectedGameObject != base.gameObject))
			{
				yield break;
			}
