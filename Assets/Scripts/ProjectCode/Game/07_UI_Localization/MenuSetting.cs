...
public class MenuSetting : MonoBehaviour
{
	...
	public void RefreshValueFromGameSettings(bool alsoApplySetting = false)
	{
		...
		case MenuSettingType.ShaderQuality:
			...
			break;
		case MenuSettingType.Dithering:
			optionList.SetOptionTo(gs.dithering);
			break;
		case MenuSettingType.Noise:
			optionList.SetOptionTo(gs.cameraNoise ? 1 : 0);
			break;
		...
		case MenuSettingType.PlayerVoice:
			...
			break;
		case MenuSettingType.NativeInput:
			optionList.SetOptionTo(gs.nativeInput ? 1 : 0);
			break;
		case MenuSettingType.XInput:
			optionList.SetOptionTo(gs.xInput ? 1 : 0);
			break;
		case MenuSettingType.MFi:
			optionList.SetOptionTo(gs.appleMFi ? 1 : 0);
			break;
		}
		if (alsoApplySetting)
		{
	...
	public void UpdateSetting(int settingIndex)
	{
		...
		case MenuSettingType.HudVisibility:
			...
			gs.hudScaleSetting = settingIndex;
			break;
		case MenuSettingType.FullScreen:
		...
		case MenuSettingType.Dithering:
			...
			gm.cameraCtrl.GetComponent<DebandEffect>().enabled = settingIndex > 1;
			break;
		case MenuSettingType.Noise:
		{
			gs.cameraNoise = settingIndex != 0;
			bool flag = gm.IsGameplayScene();
			FastNoise component = gm.cameraCtrl.GetComponent<FastNoise>();
			component.Init();
			component.enabled = flag && gm.gameSettings.cameraNoise;
			break;
		}
		case MenuSettingType.ControllerRumble:
		...
		case MenuSettingType.PlayerVoice:
			...
			break;
		case MenuSettingType.NativeInput:
			gs.nativeInput = settingIndex == 1;
			break;
		case MenuSettingType.XInput:
			gs.xInput = settingIndex == 1;
			break;
		case MenuSettingType.MFi:
			gs.appleMFi = settingIndex == 1;
			break;
		}
	}
}
