...
public class UIManager : MonoBehaviour
{
	...
	public MenuScreen advancedVideoMenuScreen;
	...
	public MenuSetting ditheringOption;

	public MenuSetting noiseOption;

	[Header("Controller Options Menu")]
	[Space(6f)]
	public MenuScreen gamepadMenuScreen;

	public MenuScreen advancedGamepadMenuScreen;

	public MenuButtonList advancedGamepadButtons;

	public MenuSetting controllerRumbleSettingAdvanced;

	public MenuSetting nativeInputSetting;

	public MenuSetting xInputSetting;

	public MenuSetting mFiSetting;
	...
	private void Start()
	{
		...
		if (gm.IsMenuScene())
		{
			...
			if (QuitToMenu.DidSaveAndQuit)
			{
				num += Platform.Current.SaveAndQuitFadeInDelay;
			}
	...
	public bool UIGoBack()
	{
		...
		return menuState switch
		{
			...
			MainMenuState.ADVANCED_GAMEPAD_MENU => advancedGamepadMenuScreen.GoBack(), 
			...
			MainMenuState.ADVANCED_VIDEO_MENU => advancedVideoMenuScreen.GoBack(), 
	...
	public void UIGoToAdvancedControllerMenu()
	{
		StartCoroutine(GoToAdvancedControllerMenu());
	}
	...
	public void UIGoToAdvancedVideoMenu()
	{
		StartCoroutine(GoToAdvancedVideoMenu());
	}
		...
		if (gs.overscanAdjusted == 1 && gs.brightnessAdjusted == 1)
		{
			...
			gm.EnsureSaveSlotSpace(delegate(bool hasSpace)
			{
				...
				else
				{
					...
					{
						ih.StartUIInput();
						SaveSlotButton saveSlotButton = gm.profileID switch
						{
							2 => slotTwo, 
							3 => slotThree, 
							4 => slotFour, 
	...
	public IEnumerator GoToControllerMenu()
	{
		...
		else if (menuState == MainMenuState.ADVANCED_GAMEPAD_MENU)
		{
			yield return StartCoroutine(HideCurrentMenu());
		}
	...
	public IEnumerator GoToAdvancedControllerMenu()
	{
		yield return StartCoroutine(HideCurrentMenu());
		yield return StartCoroutine(ShowMenu(advancedGamepadMenuScreen));
		SetMenuState(MainMenuState.ADVANCED_GAMEPAD_MENU);
	}
	...
	{
		if (menuState == MainMenuState.OPTIONS_MENU || menuState == MainMenuState.OVERSCAN_MENU || menuState == MainMenuState.BRIGHTNESS_MENU || menuState == MainMenuState.ADVANCED_VIDEO_MENU)
		{
			yield return StartCoroutine(HideCurrentMenu());
		}
	...
	public IEnumerator GoToAdvancedVideoMenu(bool rollbackRes = false)
	{
		yield return StartCoroutine(HideCurrentMenu());
		yield return StartCoroutine(ShowMenu(advancedVideoMenuScreen));
		SetMenuState(MainMenuState.ADVANCED_VIDEO_MENU);
	}
	...
	{
		if (gm.playerData.disablePause || gm.playerData.disableSaveQuit)
		{
			yield break;
		}
		ih.StopUIInput();
	...
	public void ApplyAdvancedVideoMenuSettings()
	{
	...
	public void ApplyAdvancedControllerMenuSettings()
	{
		StartCoroutine(GoToControllerMenu());
	}
	...
	public void DefaultAdvancedVideoMenuSettings()
	{
		gs.ResetAdvancedVideoSettings();
		Platform.Current.AdjustGraphicsSettings(gs);
		particlesOption.UpdateSetting(gs.particleEffectsLevel);
		shadersOption.UpdateSetting((int)gs.shaderQuality);
		ditheringOption.UpdateSetting(gs.dithering);
		noiseOption.UpdateSetting(gs.cameraNoise ? 1 : 0);
		RefreshVideoControls();
	}
	...
	public void DefaultAdvancedGamepadMenuSettings()
	{
		gs.ResetControllerSettings();
		RefreshAdvancedGamepadControls();
	}
	...
	public void LoadStoredSettings()
	{
		...
		LoadVideoSettings();
		LoadGameOptionsSettings();
		LoadAdvancedControllerSettings();
	}
	...
	private void LoadAdvancedControllerSettings()
	{
		gs.LoadControllerSettings();
		RefreshAdvancedGamepadControls();
	}
	...
	private void RefreshVideoControls()
	{
		...
		ditheringOption.RefreshValueFromGameSettings();
		noiseOption.RefreshValueFromGameSettings();
	}
	...
	private void RefreshAdvancedGamepadControls()
	{
		controllerRumbleSettingAdvanced.RefreshValueFromGameSettings(alsoApplySetting: true);
		nativeInputSetting.RefreshValueFromGameSettings();
		xInputSetting.RefreshValueFromGameSettings();
		mFiSetting.RefreshValueFromGameSettings();
		advancedGamepadButtons.SetupActive();
	}
	...
	public IEnumerator HideCurrentMenu()
	{
		...
		case MainMenuState.VIDEO_MENU:
			...
			break;
		case MainMenuState.ADVANCED_VIDEO_MENU:
			menu = advancedVideoMenuScreen;
			gs.SaveVideoSettings();
			break;
		case MainMenuState.GAMEPAD_MENU:
			...
			break;
		case MainMenuState.ADVANCED_GAMEPAD_MENU:
			menu = advancedGamepadMenuScreen;
			gs.SaveGameControllerSettings();
			break;
		case MainMenuState.KEYBOARD_MENU:
			menu = keyboardMenuScreen;
			ih.SendKeyBindingsToGameSettings();
			gs.SaveKeyboardSettings();
			ih.LoadSavedInputBindings();
			break;
