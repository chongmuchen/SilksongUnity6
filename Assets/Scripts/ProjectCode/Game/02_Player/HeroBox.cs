...
public class HeroBox : DebugDrawColliderRuntimeAdder, CustomPlayerLoop.ILateFixedUpdate
{
	...
	public void CheckForDamage(GameObject otherGameObject)
	{
		if (FSMUtility.ContainsFSM(otherGameObject, "damages_hero"))
		{
			...
			HazardType hazardType = (HazardType)FSMUtility.GetInt(fsm, "hazardType");
			heroCtrl.TakeDamage(otherGameObject, (!(otherGameObject.transform.position.x > base.transform.position.x)) ? CollisionSide.left : CollisionSide.right, damageAmount, hazardType);
		}
		else
		{
