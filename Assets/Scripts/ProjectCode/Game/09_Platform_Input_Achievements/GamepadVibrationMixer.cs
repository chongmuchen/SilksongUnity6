...
public sealed class GamepadVibrationMixer : VibrationMixer
{
	public sealed class GamepadVibrationEmission : VibrationEmission
	{
		...
		public Values GetCurrentValues()
		{
			return new Values
			{
				Small = ((target.Motors != VibrationMotors.None) ? (gamepadVibration.SmallMotor.Evaluate(timer) * Strength) : 0f),
				Large = ((target.Motors != VibrationMotors.None) ? (gamepadVibration.LargeMotor.Evaluate(timer) * Strength) : 0f)
			};
		}
	...
	{
		GamepadVibrationEmission.Values values = new GamepadVibrationEmission.Values
		{
			Small = 0f,
			Large = 0f
		};
		if (!isPaused)
		{
			...
			foreach (GamepadVibrationEmission playingEmission in playingEmissions)
			{
				...
				{
					GamepadVibrationEmission.Values values2 = playingEmission.GetCurrentValues();
					values.Small = AdjustForPlatform(Mathf.Max(values.Small, values2.Small));
					values.Large = AdjustForPlatform(Mathf.Max(values.Large, values2.Large));
