...
public class ControllerDetect : MonoBehaviour
{
	...
	private void ShowController(GamepadType gamepadType)
	{
		...
		{
			if (controllerImage2.gamepadType == gamepadType2 && (controllerImage2.platformSpecific == 0L || controllerImage2.platformSpecific.IsBitSet((int)Application.platform)))
			{
	...
	private void LookForActiveController()
	{
		...
		else if (ih.activeGamepadType != GamepadType.NONE)
		{
			ui.HideCanvasGroup(controllerPrompt);
			remapButton.gameObject.SetActive(value: true);
			ShowController((ih.ActiveGamepadAlias != GamepadType.NONE) ? ih.ActiveGamepadAlias : ih.activeGamepadType);
		}
