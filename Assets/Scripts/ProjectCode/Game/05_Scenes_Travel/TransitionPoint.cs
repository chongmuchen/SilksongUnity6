...
public class TransitionPoint : InteractableBase, ISceneLintUpgrader
{
	...
	private void TryDoTransition(Collider2D heroCollider)
	{
		...
		bool flag2 = false;
		if (instance.cState.recoiling || instance.cState.isInCutsceneMovement)
		{
			flag2 = true;
		}
		else if (instance.cState.isBinding)
		{
			if (!instance.IsShamanCrestEquipped())
			{
				flag2 = true;
			}
		}
		else if ((instance.cState.superDashing || instance.cState.superDashOnWall) && gatePosition != GatePosition.top)
		{
			flag2 = true;
		}
		else
		...
		else if (flag2 && (gatePosition == GatePosition.top || gatePosition == GatePosition.bottom))
		{
			...
			float y2 = ((gatePosition != GatePosition.top) ? (bounds3.max.y - bounds4.min.y) : (bounds3.min.y - bounds4.max.y));
