...
public class CameraController : MonoBehaviour
{
	...
	public void ApplyEffectConfiguration()
	{
		...
		component.Init();
		component.enabled = flag && flag2 && gm.gameSettings.cameraNoise;
		GetComponent<NewCameraNoise>().enabled = false;
		...
		if (flag2 && gm.gameSettings.dithering > 0)
		{
		...
		GetComponent<DebandEffect>().enabled = flag2 && gm.gameSettings.dithering > 1;
	}
	...
	public void LockToArea(CameraLockArea lockArea)
	{
		...
		if ((!(currentLockArea != null) || currentLockArea.priority <= lockArea.priority) && (!lockArea.IgnoreInSuperjump || !hero_ctrl.cState.superDashing))
		{
			...
			if (startLockedTimer > 0f && (hero_ctrl.transitionState != HeroTransitionState.WAITING_TO_TRANSITION || instantLockedArea.Contains(lockArea)))
			{
	...
	public void ReleaseLock(CameraLockArea lockarea)
	{
		...
		if (lockarea == currentLockArea)
		{
			if (lockZoneList.Count > 0)
			{
				...
				camTarget.EnterLockZone(xLockMin, xLockMax, yLockMin, yLockMax);
				if (startLockedTimer > 0f && hero_ctrl.transitionState != HeroTransitionState.WAITING_TO_TRANSITION)
				{
					Vector3 position = hero_ctrl.transform.position;
					position.x += camTarget.xOffset;
					camTarget.transform.SetPosition2D(KeepWithinLockBounds(position));
			...
			else
			{
				...
				if (!hero_ctrl.cState.hazardDeath && !hero_ctrl.cState.dead && gm.GameState != GameState.EXITING_LEVEL && mode != CameraMode.FROZEN)
				{
					SetMode(CameraMode.FOLLOWING);
				}
				if (startLockedTimer > 0f)
