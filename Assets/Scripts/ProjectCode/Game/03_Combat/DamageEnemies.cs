...
public class DamageEnemies : DebugDrawColliderRuntimeAdder, CustomPlayerLoop.ILateFixedUpdate
{
	...
	public bool DoDamage(GameObject target, bool isFirstHit = true)
	{
		...
		try
		{
			...
			if (useNailDamage || useHeroDamageAffectors)
			{
				if (useNailDamage)
				{
					tempDamageStack.SetupNew(Mathf.RoundToInt((float)PlayerData.instance.nailDamage * nailDamageMultiplier));
				}
			...
			if ((bool)representingTool && representingTool.Type == ToolItemType.Skill && Gameplay.ZapImbuementTool.Status.IsEquipped)
			{
			...
			HitInstance hitInstance = new HitInstance
			{
				...
				NailElement = (NailImbuement ? NailElement : NailElements.None),
				NailImbuement = NailImbuement,
				IsUsingNeedleDamageMult = (useNailDamage || useHeroDamageAffectors),
				RepresentingTool = representingTool,
				PoisonDamageTicks = PoisonDamageTicks,
				ZapDamageTicks = ZapDamageTicks,
				DamageScalingLevel = damageScalingLevel,
				ToolDamageFlags = (toolItem ? toolItem.DamageFlags : damageFlags),
				CircleDirection = (directionSourceOverride == DirectionSourceOverrides.CircleDirection),
				DamageDealt = num5,
				StunDamage = (doesNotStun ? 0f : stunDamage),
				CanWeakHit = canWeakHit,
				Direction = num6,
				UseCorpseDirection = corpseDirection.IsEnabled,
				CorpseDirection = corpseDirection.Value,
				CanTriggerBouncePod = canTriggerBouncePod,
				ExtraUpDirection = ExtraUpDirection,
				IgnoreInvulnerable = ignoreInvuln,
				MagnitudeMultiplier = num2,
				UseCorpseMagnitudeMult = corpseMagnitudeMult.IsEnabled,
				CorpseMagnitudeMultiplier = corpseMagnitudeMult.Value,
				UseCurrencyMagnitudeMult = currencyMagnitudeMult.IsEnabled,
				CurrencyMagnitudeMult = currencyMagnitudeMult.Value,
				MoveAngle = 0f,
				MoveDirection = moveDirection,
				Multiplier = 1f,
				SpecialType = specialType,
				SlashEffectOverrides = ((slashEffectOverrides.Length != 0) ? slashEffectOverrides : null),
				HitEffectsType = hitEffectsType,
				SilkGeneration = silkGeneration,
				NonLethal = nonLethal,
				RageHit = rageHit,
				CriticalHit = criticalHit,
				HunterCombo = hunterCombo,
				UseBouncePodDirection = useBouncePodDirection,
				BouncePodDirection = bouncePodDirection,
				IsManualTrigger = manualTrigger,
				IsHeroDamage = (isHeroDamage || sourceIsHero),
				IsNailTag = isNailAttack,
				IgnoreNailPosition = IgnoreNailPosition
			};
			bool flag3 = false;
			PhysLayers layer;
			if (healthManager != null)
			{
				...
				if (flag2)
				{
					...
					this.WillDamageEnemyOptions?.Invoke(healthManager, hitInstance);
				}
			}
			else
			{
	...
	private void DoEnemyDamageNailImbuement(HealthManager healthManager, HitInstance hitInstance)
	{
		...
		{
			inertHitEffect.SpawnEffects(healthManager.transform, healthManager.EffectOrigin, hitInstance);
		}
		HeroController instance = HeroController.instance;
		...
		{
			startHitEffect.SpawnEffects(healthManager.transform, healthManager.EffectOrigin, hitInstance);
		}
	...
	public void SetSilkGenerationFull()
	{
		silkGeneration = HitSilkGeneration.Full;
	}

	public void SetSilkGenerationNone()
	{
		silkGeneration = HitSilkGeneration.None;
	}
