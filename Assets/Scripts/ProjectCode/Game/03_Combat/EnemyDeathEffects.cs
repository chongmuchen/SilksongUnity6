...
public class EnemyDeathEffects : MonoBehaviour, IInitialisable, BlackThreadState.IBlackThreadStateReceiver
{
	...
	protected GameObject EmitCorpse(float? attackDirection, float flingMultiplier, AttackTypes attackType, NailElements nailElement, GameObject damageSource, Action<Transform> onCorpseBegin, out bool didCallCorpseBegin)
	{
			...
			if (blackThreadEffects != null && blackThreadEffects.Length > 0)
			{
				...
				if (IsCorpseRecyclable)
				{
					RecycleResetHandler.Add(gameObject, (Action)delegate
					{
						if (blackThreadEffects != null && blackThreadEffects.Length > 0)
						{
							BlackThreadState.IBlackThreadStateReceiver[] array3 = blackThreadEffects;
							for (int j = 0; j < array3.Length; j++)
							{
								array3[j]?.SetIsBlackThreaded(isThreaded: false);
							}
						}
					});
				}
		...
		else
		{
			...
			float[] array2 = new float[componentsInChildren.Length];
			float num3 = 0f;
			for (int num4 = 0; num4 < componentsInChildren.Length; num4++)
			{
				Collider2D component8 = componentsInChildren[num4].GetComponent<Collider2D>();
				if ((bool)component8)
				{
					num3 += (array2[num4] = component8.bounds.size.magnitude);
				}
				else
				{
					array2[num4] = 0f;
			...
			{
				ActiveCorpse obj2 = componentsInChildren[num5];
				float scale = array2[num5] / num3;
		...
		{
			flingMultiplier = 0f;
			num6 = Mathf.Sign(base.transform.lossyScale.x);
		}
		int num7 = DirectionUtils.GetCardinalDirection(attackDirection.GetValueOrDefault());
		float num8 = gameObject.transform.localScale.x * (corpseFacesRight ? 1f : (-1f)) * num6;
		...
		{
		case 0:
			num12 = (lowCorpseArc ? 10f : num10);
			gameObject.transform.SetScaleX((0f - num8) * Mathf.Sign(base.transform.localScale.x));
			break;
		case 2:
			num12 = (lowCorpseArc ? 170f : num11);
			gameObject.transform.SetScaleX(num8 * Mathf.Sign(base.transform.localScale.x));
			break;
		case 3:
			num12 = 270f;
			break;
		case 1:
			num12 = UnityEngine.Random.Range(75f, 105f);
			num9 *= 1.3f;
			break;
		default:
			num12 = 90f;
		...
		component9.linearVelocity = new Vector2(Mathf.Cos(num12 * (MathF.PI / 180f)), Mathf.Sin(num12 * (MathF.PI / 180f))) * (num9 * flingMultiplier);
		return gameObject;
	}
